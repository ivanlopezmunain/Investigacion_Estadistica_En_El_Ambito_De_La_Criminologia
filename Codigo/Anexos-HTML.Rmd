...
Los siguientes apartados se encuentran nombrados igual que en la documentación del proyecto, así es posible localizar los fragmentos de código de cada uno de los procedimientos llevados a cabo. Al tratarse de un _notebook_ _html_ permite las representaciones interactivas y dinámicas, es por esto que se recomienda utilizar este soporte en vez de los anexos en _pdf_. Esta documentación se ha realizado mediante __Rmarkdown__. 

__Es recomendable utilizar esta plataforma en vez del pdf debido a que contiene mayor cantidad de información.__
...
paged_table(datosEuropa[,c("TIME","GEO","ICCS","Value")])
...
#ademas no coger el total de poblacion en EspaÃ±a
...
#"C: Delitos de lesiones", "D: Secuestro", 
#"E: Delitos contra la libertad e indemnidad sexual",
#"F: Agresion sexual con penetraciÃ³n",
#"G: Resto de delitos contra la libertad",
#"H: Robos con violencia e intimidaciÃ³n",
#"I: Robos con fuerza","J: Robos en domicilios",
#"K: SustracciÃ³n de vehÃ�culos","L: TrÃ¡fico de drogas")

ind <- c("A","B","C","D","E","F","G","H","I","J","K","L","M","N")
#ind <- c("A","B","C","D","E","F","G","H","I","J","K","L")

finalTipos<-data.frame(ClasificaciÃ³n=names,
...
graf<-ggplot(data=finalTipos, 
             aes(x=Identificador,
                 y=NumInfr, fill=ClasificaciÃ³n)) + 
  geom_bar(stat="identity", position="dodge")

graf<-graf + 
  ggtitle("Distribución de los delitos según su naturaleza") +
  xlab("Tipos de infracciones") + 
  ylab("Número de infracciones") +
  theme(plot.title = element_text(hjust = 0.5, face="bold"))

#con ggplotly se puede observar quÃ© tipos quieres ver, zoom, etc
#ggplotly(graf)
graf
...
edad<-c("14-17","18-30","31-40","41-64","MÃ¡s de 64")
...
#  links$value[which(
#    links$source=="Agresion sexual con penetraciÃ³n")])]
...
ggplotly(grafExtr + geom_jitter(shape=16,
...
edad<-rep(c("14-17","18-30","31-40","41-64","MÃ¡s de 64"),19)
...
edad<-as.factor(
  rep(rep(
    c("14-17","18-30","31-40","41-64","MÃ¡s de 64"),each=2), 9))
anio<-(rep
...
nombresDelitos <- c("Asesinatos consumados",
                    "Asesinatos en grado tentativa", 
                    "Delitos de lesiones", "Secuestro",
                    "Delitos contra la libertad e indemnidad sexual", "Agresion sexual con penetracion",
                    "Resto de delitos contra la libertad",
...
grafHist<-ggplot(data = dataProc, aes(x = Infracciones)) +
  ...
  ggtitle("Histograma / Curva normal teÃ³rica") +
  theme_bw()+
  theme(plot.title = element_text(hjust = 0.5,face="bold"))
...
#manual (ejemplo)
#chis<-2*(logLik(modSexoEdadAÃ±oNB)-logLik(modSexoEdadNB))
#1-pchisq(chis[1],1)
...
#descartamos mod2
#del resto nos quedamos con mod3 que es el Ãºnico vÃ¡lido

#anova(modSexoEdadAÃ±oNB, mod1)
#anova(modSexoEdadAÃ±oNB, mod2)
#anova(modSexoEdadAÃ±oNB, mod3)
#anova(modSexoEdadAÃ±oNB, mod4)
...
auxDf<-data.frame(Estimacion=coef(mod3)[order(coef(mod3))],
                  Covariable=c("Intercept","Edad +64",
                               ...
                               "Año","Edad41-64",
                                ...
                                "SexoMasculino")) %>%
  mutate(
    Correlacion=ifelse(Estimacion>0,"Positiva","Negativa"))%>% 
  arrange(Estimacion) %>% 
  mutate(Covariable=factor(Covariable, levels = Covariable))

p <- ggplot(auxDf, aes(x = Covariable, y = Estimacion)) +
  geom_col(aes(fill = Correlacion), width = 0.7)
...
#Capa de salida de un perceptrÃ³n multicapa
SRNN.add(Dense(1)) 
...
![Red neuronal recursiva simple](./Graficos/grafRNRSimple.PNG){width=75%}
...
![Red neuronal recursiva LSTM](./Graficos/grafRNRlstm.PNG){width=75%}
...
#CODIFICACION:
#A: tasa de criminalidad hombres
#B: tasa de criminalidad mujeres
#C: tasa de paro hombres
#D: tasa de paro mujeres
